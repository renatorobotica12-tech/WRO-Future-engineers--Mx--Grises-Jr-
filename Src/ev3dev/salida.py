#!/usr/bin/env python3
"""Parking exit: back out of a bay and point the robot at the gap.
...
def lado_elegido():
    ...
    Falls back to the configured side until elegir_lado has run, so
    anything asking early gets an answer rather than None.
    """
    return _LADO or config.SALIDA_LADO
...
def elegir_lado(hub):
    ...
    if not config.SALIDA_LADO_AUTOMATICO:
        _LADO = config.SALIDA_LADO
        print('lado de salida   : %s (fijo en config)' % _LADO.upper())
        return _LADO

    izq = []
    der = []
    fin = time.time() + config.SALIDA_LADO_MUESTREO
    while time.time() < fin:
        hub.actualizar()
...
def main():
    ...
    print('retroceso        : %.1f cm a la %s%s, %.0f grados de encoder'
          % (config.SALIDA_RETROCESO_CM, config.SALIDA_LADO,
             ' salvo que los sensores digan otra cosa'
             if config.SALIDA_LADO_AUTOMATICO else ' (fijo)',
             config.SALIDA_RETROCESO_CM
